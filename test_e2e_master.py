...
def main():
    ...
    ai_prompt = {
        "userId": "admin-1",
        "prompt": f"Draft an exclusive Winter Clearance final-day campaign for our most engaged real_time_test users from Rare Rabbit. Focus on premium heavy jackets and deep winter discounts. For the segment SQL, you MUST use exactly this: SELECT id FROM customers WHERE 'real_time_test' = ANY(tags). Ensure you append this timestamp ({timestamp}) to the names of any segments or campaigns you create so they are perfectly unique."
    }
