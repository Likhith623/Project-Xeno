...
print("Starting explicit Live Cloud Email Dispatch...")

seg_payload = {
    "name": f"Direct Send {int(time.time())}",
    "type": "DYNAMIC",
...
print("Evaluating segment... Waiting 15 seconds.")
time.sleep(15)
...
print(f"Segment Evaluated. Members matched: {len(members)}")

camp_payload = {"name": f"Live Cloud Delivery Test {int(time.time())}", "segmentId": seg_id, "goal": "Deliver"}
...
request("POST", f"/campaigns/{camp_id}/execute")

print("Waiting 15 seconds for SMTP dispatch...")
time.sleep(15)
