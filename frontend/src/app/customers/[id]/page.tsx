...
export default function Customer360() {
  ...
  if (isError || !customer360) {
    return (
      <div className="flex h-screen w-full bg-[#F1F3F5] overflow-hidden font-sans">
        <Sidebar />
        <div className="flex-1 flex flex-col items-center justify-center min-w-0 overflow-hidden gap-4">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="h-[56px] bg-white border-b border-border-primary flex items-center justify-between px-6 shrink-0">
          <div className="flex items-center gap-3">
            <Link href="/customers" className="w-8 h-8 rounded-lg border border-border-primary flex items-center justify-center text-text-secondary hover:bg-bg-secondary">
              <ArrowLeft className="w-4 h-4" />
            </Link>
            <h1 className="text-[15px] font-medium text-text-primary">Customer 360 — {c.name || 'Unknown'}</h1>
          </div>
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" className="h-8 text-[13px] gap-2">
              <MessageSquarePlus className="w-4 h-4" /> Send message
            </Button>
            <Button variant="outline" size="sm" className="h-8 text-[13px] gap-2" onClick={() => updateMutation.mutate()} disabled={updateMutation.isPending}>
             ...
             <Button variant="outline" size="sm" className="h-8 text-[13px] gap-2 text-red-600 border-red-200 hover:bg-red-50 hover:text-red-700" onClick={() => deleteMutation.mutate()} disabled={deleteMutation.isPending}>
              <Trash className="w-4 h-4" /> Delete
            </Button>
          </div>
        </header>

        <main className="flex-1 overflow-y-auto grid grid-cols-1 md:grid-cols-[300px_1fr] min-h-0">
          {/* Left Panel */}
          <div className="bg-white border-r border-border-primary p-6 flex flex-col gap-6 overflow-y-auto">
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 rounded-full bg-brand-light text-brand flex items-center justify-center text-xl font-medium shrink-0">
                {(c.name || 'U').split(' ').map((n: string) => n[0]).join('')}
              </div>
              <div>
                <div className="text-[17px] font-medium text-text-primary mb-1">{c.name || 'Unknown'}</div>
                <div className="text-[13px] text-text-secondary">{c.city || 'N/A'} · VIP</div>
              </div>
            </div>

            <div className="flex flex-col text-[13px]">
              <DetailRow label="Email" value={<span className="text-brand">{c.email}</span>} />
                   ...
                   (orders as any[]).map((o: any) => {
                     ...
                     return (
                       <div key={o.id} className="flex items-center gap-3 py-2 border-b border-border-tertiary last:border-0">
                         <span className="font-medium w-[120px] shrink-0">{o.orderNumber || o.id.substring(0,8)}</span>
                         ...
                         <span className="font-medium min-w-[56px] text-right">${o.totalAmount?.toFixed(2)}</span>
                       </div>
                     );
                   })
                 )}
              ...
              <CardContent className="px-5 pb-2 text-[13px]">
                 {!communications || (communications as any[]).length === 0 ? (
                   ...
                   (communications as any[]).map((comm: any) => {
                     ...
                     return (
                       <div key={comm.id} className="flex items-center gap-3 py-2 border-b border-border-tertiary last:border-0">
                         ...
                         <span className="text-text-tertiary text-[11px] min-w-[70px] text-right">
