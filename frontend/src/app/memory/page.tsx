...
import { Brain, RefreshCcw, Bot, Search, Send, Loader2, Sparkles } from "lucide-react";
...
export default function MemoryPage() {
  ...
  // GET /memory/query?segmentTag=&channel= — Filtered memories
  ...
  const chatMutation = useMutation({
    ...
    onSuccess: (data: any, variables) => {
      ...
      setChatHistory([...chatHistory, { role: 'user', content: variables }, { role: 'agent', content: data?.textReply || `Agent started session ${sid || 'successfully'}. Check the Agent Workspace for real-time tracking.` }]);
      toast.success("Agent started working on your request.");
      setChatInput("");
    },
    onError: (err: any) => {
        ...
        <div className="flex flex-col gap-6">
          <Card className="shadow-minimal border-border-primary">
            ...
            <CardContent className="px-5 pb-5">
              ...
              <div className="text-[11px] text-text-tertiary mb-4">Showing: all channels · all segments</div>
              ...
              <div className="flex flex-col gap-2 max-h-[500px] overflow-y-auto pr-2">
                {isLoadingMemory ? (
                  ...
                  <div className="text-center py-8 text-text-tertiary text-[13px]">No insights found.</div>
                ) : (
                  memories.map((m: any) => (
                    <MemoryCard
                      key={m.id}
                      channel={m.channel || 'Unknown'}
                      chColor={m.channel === 'whatsapp' ? 'bg-green-50 text-green-700' : m.channel === 'sms' ? 'bg-purple-50 text-purple-700' : 'bg-blue-50 text-blue-700'}
        ...
        <div className="flex flex-col gap-6">
          <Card className="shadow-minimal border-border-primary flex-1 flex flex-col min-h-[500px]">
            ...
            <CardContent className="px-5 pb-5 flex-1 flex flex-col justify-between">
              <div className="flex flex-col gap-4 overflow-y-auto mb-4 p-1 max-h-[400px]">
                {/* Intro Msg */}
                <div>
                ...
                {chatHistory.map((msg, idx) => (
                  <div key={idx} className={msg.role === 'user' ? "self-end w-11/12 flex justify-end" : ""}>
                    ...
                    <div className={msg.role === 'user' ? "bg-brand-light text-brand p-3 rounded-2xl rounded-tr-sm text-[13px] leading-relaxed inline-block" : "bg-bg-secondary border border-border-tertiary text-text-primary p-3 rounded-2xl rounded-tl-sm text-[13px] leading-relaxed w-11/12"}>
                      {msg.content}
                    </div>
                  </div>
                ))}
              ...
              <div className="flex items-center gap-2 pt-3 border-t border-border-tertiary shrink-0">
                ...
                />
                <Button onClick={handleSend} disabled={!chatInput.trim() || chatMutation.isPending} className="shrink-0 h-[38px] w-[38px] p-0"><Send className="w-4 h-4" /></Button>
              </div>
            </CardContent>
          </Card>
        </div>
  ...
  return (
    <div className="bg-bg-secondary rounded-xl p-3.5 hover:bg-[#ebedef] transition-colors cursor-pointer">
      <div className="flex items-center gap-2 mb-2">
        <Badge className={`${chColor} hover:${chColor} font-medium text-[10px] px-2 h-5`}>{channel}</Badge>
