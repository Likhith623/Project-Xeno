...
Based on a rigorous analysis of all `@RestController` controllers in the Xeno CRM backend, here is the exhaustive list of every active API endpoint, its path, its exact operational purpose, and the input/output schemas.
...
* **`POST /chat`**
  * **Purpose:** Main entry point for the Sovereign Agent. Takes a natural language request from the user (e.g., "Send a discount to VIPs"), communicates with Google Gemini, translates the intent into an SQL segment, drafts message variants, and returns an execution plan.
  * **Input:** `{"prompt": "string"}`
  * **Output:** `{"sessionId": "UUID", "status": "IN_PROGRESS", "textReply": "string"}`
* **`GET /sessions/{sessionId}`**
  * **Purpose:** Retrieves the current state and conversation log of an active or completed AI session. Poll this until `status` is `COMPLETED`.
  * **Output:** `{"id": "UUID", "goal": "string", "status": "COMPLETED", "plan": {"segmentId": "UUID", "campaignId": "UUID", "channelRecommendation": {}}}`
...
* **`PUT /{id}`**
  ...
  * **Input:** Customer Input Object
* **`DELETE /{id}`**
  * **Purpose:** Hard deletes a customer record from the database.
* **`GET /by-email`**
  * **Purpose:** Looks up a specific customer's UUID by providing their email address.
  * **Query Params:** `email`
* **`GET /by-tag`**
...
* **`POST /`**
  * **Purpose:** Records a new transaction/order. Successfully inserting an order triggers asynchronous processes to recalculate the buyer's metrics (e.g., updating their monetary total) and trigger Revenue Attribution for campaigns.
  * **Input:** `{"customerId": "UUID", "orderNumber": "string", "totalAmount": float, "currency": "string", "items": [{"productId": "UUID", "quantity": int, "unitPrice": float}]}`
  * **Output:** Order Entity
* **`POST /bulk`**
...
* **`PATCH /{id}`**
  * **Purpose:** Update a segment's criteria (JSON/SQL) or description.
...
* **`POST /{id}/execute`**
  * **Purpose:** **The Go-Live Trigger.** Executes a fully prepared campaign. It resolves the attached segment, fetches matching customer profiles, applies the message variants, and pushes the jobs to the asynchronous `ChannelDispatchService`.
...
## 9. Callbacks API (`/api/v1/callbacks/channel`)
* **`POST /`**
  * **Purpose:** Webhook receiver for the Channel Stub. Receives delivery receipts and read-receipts to update internal variant metrics.
  * **Input:** `{"channelMessageId": "string", "status": "DELIVERED|OPENED|CLICKED|FAILED", "metadata": {}}`

## 10. AI Memory & Learning (`/api/v1/memory`)
* **`GET /`**
...
## 12. AI Corrections (`/api/v1/corrections`)
* **`GET /`**
