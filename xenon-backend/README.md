# Project Xeno: AI-Native CRM Backend

Welcome to the backend repository of **Project Xeno**, the world's first multi-agent autonomous CRM.

## 🏗️ Architecture & Technology Stack

The backend is built as a modular monolith using **Java 17** and **Spring Boot 3.3.0**. 

**Core Stack:**
- **Framework:** Spring Boot (Web, Security, Data JPA, Validation)
- **Database:** PostgreSQL 15+ (with Flyway migrations)
- **AI Engine:** Google Gemini 2.5 Flash API (ReAct prompt architecture)
- **Math Engine:** Apache Commons Math 3 (Thompson Sampling Beta Distribution)

**Key Modules:**
- `agent/`: Sovereign AI Agent orchestrator using ReAct prompting.
- `campaign/`: Campaign execution engine, `@Async` dispatch, and `@Scheduled` AI jobs.
- `segment/`: Dynamic SQL generation for audience segmentation.
- `variant/`: Multi-Armed Bandit (MAB) A/B testing and LLM-powered variant breeding.
- `channelservice/`: Smart routing for Email, SMS, WhatsApp, and RCS.

## 🚀 Setup & Installation

1. **Prerequisites**: Java 17, Maven (or use `./mvnw`), PostgreSQL 15+.
2. **Database**: Create a PostgreSQL database (e.g., `xeno_crm`).
3. **Environment Variables** (set these or configure `application.yml`):
   - `DB_URL`, `DB_USERNAME`, `DB_PASSWORD`
   - `GEMINI_API_KEY`: Your Google Gemini API key.
   - `XENO_API_KEY`: The master key for frontend integration (default: `likhit@178926a`).
4. **Run**:
   ```bash
...
## Exhaustive Frontend Integration Guide

This README contains API contracts for **EVERY SINGLE ENDPOINT** present in the codebase.For every feature, you will find exactly what it does, the problem it solves, how it is technically implemented, exact API payloads, a real-world scenario example, and its strategic business importance.
